...
class MembersController extends Controller
{
    ...
    public function launch(Request $request)
    {
        ...
        $memberTotal = Member::indexQuery($request->sort_field, $request->sort_direction, $request->drp_start, $request->drp_end)->search('"'.$request->input('search').'"')->get();
        $count = $memberTotal->count();
        //$testval = $members->count();
        //echo("<script>console.log('".json_encode($testval)."');</script>");
        //echo("<script>console.log('".json_encode("event2")."');</script>");
        if (! $request->has('drp_start') or ! $request->has('drp_end')) {
            $drp_placeholder = '选择日期范围';
        } else {
            $drp_placeholder = $request->drp_start.' - '.$request->drp_end;
        ...
        // $member = Member::findOrFail(5);
        // $member->update($request->all());

        // $member = Member::findOrFail(5);
        // $member_number_mode = \Utilities::getSetting('member_number_mode');
        // $member_code = $member->member_code;
        // echo("<script>console.log('".json_encode("n111b")."');</script>");
        ...
        return view('members.launch', compact('members', 'count', 'drp_placeholder', 'old_sort'));
    }
    ...
    public function logout(Request $request)
    {
        ...
        return view('members.logout', compact('members', 'count', 'drp_placeholder', 'old_sort'));
    }

    public function showMemSts(Request $request)
    {
        // echo("<script>console.log('".json_encode($request)."');</script>");
        ...
        return view('members.showMemSts', compact('members', 'count', 'drp_placeholder', 'old_sort'));
    }
    ...
    public function updatePinCode($id, Request $request)
    {
        //dd($request->all());
        $member = Member::findOrFail($id);
        $member->update($request->all());
        ...
        flash()->success('無右健康，开始享受健身吧');
        //echo("<script>console.log('".json_encode("hi")."');</script>");
        return redirect(action('MembersController@showMemSts', ['id' => $member->id]));
    }
