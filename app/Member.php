...
class Member extends Model implements HasMediaConversions
{
    ...
    public function scopeIndexQuery($query, $sorting_field, $sorting_direction, $drp_start, $drp_end)
    {
        ...
        if ($drp_start == null or $drp_end == null) {
            return $query->select('mst_members.id', 'mst_members.member_code', 'mst_members.name', 'mst_members.pin_code', 'mst_members.contact', 'mst_members.created_at', 'mst_members.status')->where('mst_members.status', '!=', \constStatus::Archive)->orderBy($sorting_field, $sorting_direction);
        }

        return $query->select('mst_members.id', 'mst_members.member_code', 'mst_members.name', 'mst_members.pin_code', 'mst_members.contact', 'mst_members.created_at', 'mst_members.status')->where('mst_members.status', '!=', \constStatus::Archive)->whereBetween('mst_members.created_at', [$drp_start, $drp_end])->orderBy($sorting_field, $sorting_direction);
    }
